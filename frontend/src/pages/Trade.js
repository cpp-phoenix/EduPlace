...
function Trade () {
    ...
    const[EDPTokenAmount, setEDPTokenAmount] = useState(0);
    const[EDPTokenBalance, setEDPTokenBalance] = useState(0);
    const[selectedCount, setSelectedCount] = useState(0);

    const[EDPTokenToSell, setEDPTokenToSell] = useState(0);
    ...
    const loadNFTData = async () => {
        ...
        const _EDPTokenBalance = await EDPTokenContract.balanceOf(address)

        setEDPTokenBalance(ethers.formatEther(_EDPTokenBalance))
    ...
    const initiateSwap = async () => {
        if(selectedSell === "NFT") {
            if(selectedCount > 0) {
                ...
                await signedMarketplaceContract.swapNFT(NFT_CONTRACTS[chain.id][nftID].contractAddress, NFTList)
            }
        }
    }
    ...
    const estimateNFTSell = async () => {
        if(selectedSell === "NFT") {
            if(selectedCount > 0) {
                ...
                const estimatedAmount = await MarketplaceContract.estimateSwapNFT(NFT_CONTRACTS[chain.id][nftID].contractAddress, NFTList)
                setEDPTokenAmount(ethers.formatEther(estimatedAmount.toString()))
            } else {
                setEDPTokenAmount(0)
            }
        }
    ...
    const quoteAmountNftReceived = async (amount) => {
        if(selectedSell === "TOKEN") {
            if(amount > 0) {
                ...
                const nftReceived = await MarketplaceContract.quoteSwapToken(NFT_CONTRACTS[chain.id][nftID].contractAddress, ethers.parseEther(amount,"ether"))
    ...
    const initiateSwapToken = async () => {
        if(selectedSell === "TOKEN") {
            if(estimateReceiveNFTs > 0 && EDPTokenToSell > 0) {
                const provider = new ethers.JsonRpcProvider(NFT_CONTRACTS[chain.id].rpc);
                const MarketplaceContract = new ethers.Contract(NFT_CONTRACTS[chain.id].Marketplace, MainABI, provider)
                const EDPTokenContract = new ethers.Contract(NFT_CONTRACTS[chain.id].EDPToken, ERC20ABI, provider);
                ...
                try {
                    if(tokenAllowance < EDPTokenToSell) {
                        await signedEDPTokenContract.approve(NFT_CONTRACTS[chain.id].Marketplace, ethers.parseUnits(EDPTokenToSell, "ether"))
                    }
                    await signedMarketplaceContract.swapToken(NFT_CONTRACTS[chain.id][nftID].contractAddress,ethers.parseEther(EDPTokenToSell,"ether"))
                } catch (e) {
                    ...
                    <div className="w-full space-y-5"> 
                        ...
                        <div className="w-full space-y-1">
                            ...
                            <div className="flex justify-between bg-[rgb(14,19,32,0.3)] w-full h-[70px] rounded-[12px] p-4">
                                ...
                                <input value={EDPTokenAmount} className="w-20 bg-[rgb(14,19,32,0.3)]" placeholder="0.00" disabled/>
                            </div>
                            <div className="text-end text-xs font-semibold">Balance: {EDPTokenBalance}</div>
                        </div>
                    ...
                    <div className="w-full space-y-5"> 
                        <div className="w-full space-y-1">
                            ...
                            <div className="flex justify-between bg-[#19232D] w-full h-[70px] rounded-[12px] p-4">
                                ...
                                <input onChange={(e) => {setEDPTokenToSell(e.target.value); quoteAmountNftReceived(e.target.value);}}  className="w-20 bg-[#19232D]" placeholder="0.00"/>
                            </div>
                            <div className="text-end text-xs font-semibold">Balance: {EDPTokenBalance}</div>
                        </div>
                        <div class="relative flex justify-center">
